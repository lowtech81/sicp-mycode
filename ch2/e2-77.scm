...
(define test-complex1 (make-complex-from-real-imag 3 4))
(define test-complex2 (make-complex-from-real-imag 5 12))

(apply-generic 'add  test-complex1 test-complex2) ;Value: (complex rectangular 8 . 16）

; The magnitude selector for our new complex type, which was tagged 'complex'
; and 'rectangular', is not in the generic table, so the next call will gets error
(apply-generic 'magnitude test-complex1)

; now we add selector to the table
(put 'real-part '(complex) real-part)
(put 'imag-part '(complex) imag-part)
(put 'magnitude '(complex) magnitude)
(put 'angle '(complex) angle)

; call magnitude again, should return 5
(apply-generic 'magnitude test-complex1) 

; two (appli-generic) invoked
; (apply-generic 'magnitude '(complex rectangular 3.4)) ->  (apply-generic 'magnitude (rectangular 3.4)))
